...
def process_pair(args: tuple) -> tuple:
    ...
    try:
        ...
        # to calibrate the pixels, and the zeropoints are far apart (VIS 24.5 vs
        # NIR_H 30.0). Leaving this at "VIS" while pointing at nir_h cutouts
        # scales them ~158x too bright, which arcsinh then compresses into
        # saturated, contrast-free blobs. Change both lines together.
        euc = preprocess_image_v2(euc_tensor, crop_size=EUCLID_CROP_SIZE, bands=["NIR_H"]).squeeze(0).numpy()
...
def main():
    ...
    # magnitude cut, restricted to the B tiles. na=False so a missing tile is
    # dropped rather than raising on a NaN in the boolean mask.
    #catalog = catalog[(catalog["mag_model_f150w"] < 25) & (catalog["tile"].str.startswith("B", na=False))].reset_index(drop=True)
    #print(f"Pairs after magnitude + tile-B cut: {len(catalog)}")
